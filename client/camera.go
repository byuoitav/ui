...
func (c *client) tiltUp(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.TiltUp)
			return
		}
	}
...
func (c *client) tiltDown(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.TiltDown)
			return
		}
	}
...
func (c *client) panLeft(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.PanLeft)
			return
		}
	}
...
func (c *client) panRight(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.PanRight)
			return
		}
	}
...
func (c *client) panTiltStop(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.PanTiltStop)
			return
		}
	}
...
func (c *client) zoomIn(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.ZoomIn)
			return
		}
	}
...
func (c *client) zoomOut(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.ZoomOut)
			return
		}
	}
...
func (c *client) zoomStop(data []byte) {
	...
	for _, cam := range cg.Cameras {
		if cam.Name == msg.Camera {
			...
			_ = c.doStateTransition(ctx, nil, cam.ZoomStop)
			return
		}
	}
		...
		if cam.Name == msg.Camera {
			for _, preset := range cam.Presets {
				if preset.Name == msg.Preset {
					...
					_ = c.doStateTransition(ctx, nil, preset.SetPreset)
					return
				}
			}
